...
func TestTestNodeDelayRejectsNegativeDelay(t *testing.T) {
	app, _ := newTestApp(t)
	cfg := config.Default()
	app.Client = &http.Client{
		Transport: roundTripFunc(func(req *http.Request) (*http.Response, error) {
			return textResponse(http.StatusOK, `{"delay":-1}`), nil
		}),
	}
	if _, err := app.testNodeDelay(cfg, "delay-node"); err == nil || !strings.Contains(err.Error(), "invalid delay") {
		t.Fatalf("expected invalid delay error, got %v", err)
	}
}
...
func TestServiceMenuRetriesAfterInvalidChoice(t *testing.T) {
	app, _ := newTestApp(t)
	if err := app.serviceMenu(bufio.NewReader(strings.NewReader("x\n4\n"))); err != nil {
		t.Fatalf("service menu retry status: %v", err)
	}
	output := app.Stdout.(*bytes.Buffer).String()
	for _, needle := range []string{
		"无效选择",
...
func TestServiceMenuDispatchesStart(t *testing.T) {
	...
	if err := app.serviceMenu(bufio.NewReader(strings.NewReader("1\n"))); err != nil {
		t.Fatalf("service menu start: %v", err)
	}
	if !hasRecordedCall(calls, "systemctl", "enable", "--now", "minimalist.service") {
		t.Fatalf("expected start command to enable minimalist.service, calls=%#v", calls)
	}
	if !strings.Contains(app.Stdout.(*bytes.Buffer).String(), "已生成 ") {
		t.Fatalf("expected render output before service start:\n%s", app.Stdout.(*bytes.Buffer).String())
	}
}

func TestServiceMenuDispatchesRestart(t *testing.T) {
	app, _ := newTestApp(t)
	oldGeteuid := geteuid
	geteuid = func() int { return 0 }
	defer func() { geteuid = oldGeteuid }()
	...
	if err := app.serviceMenu(bufio.NewReader(strings.NewReader("2\n"))); err != nil {
		t.Fatalf("service menu restart: %v", err)
	}
	if !hasRecordedCall(calls, "systemctl", "restart", "minimalist.service") {
		t.Fatalf("expected restart command for minimalist.service, calls=%#v", calls)
	}
	if !strings.Contains(app.Stdout.(*bytes.Buffer).String(), "已生成 ") {
		t.Fatalf("expected render output before service restart:\n%s", app.Stdout.(*bytes.Buffer).String())
	}
}
...
func TestNodesMenuDispatchesEnableAndDisableNode(t *testing.T) {
	app, _ := newTestApp(t)
	app.Stdin = strings.NewReader("trojan://password@example.org:443?security=tls#toggle-menu-node\n")
	if err := app.ImportLinks(); err != nil {
		t.Fatalf("import links: %v", err)
	}
	...
	if st.Nodes[0].Enabled {
		t.Fatalf("expected node disabled via menu, got %+v", st.Nodes[0])
	}
}
...
func TestApplyRulesDefaultConfigProgramsLANDNSHijackWithoutHostOutputJump(t *testing.T) {
	app := newTestAppWithEnabledManualNode(t)
	oldGeteuid := geteuid
	geteuid = func() int { return 0 }
	defer func() { geteuid = oldGeteuid }()
	...
	for _, expect := range []struct {
		...
		{"iptables", []string{"-w", "5", "-t", "nat", "-A", "MIHOMO_DNS", "-i", "bridge1", "-j", "MIHOMO_DNS_HANDLE"}},
		{"iptables", []string{"-w", "5", "-t", "nat", "-A", "MIHOMO_DNS_HANDLE", "-p", "udp", "--dport", "53", "-j", "REDIRECT", "--to-ports", "1053"}},
		{"iptables", []string{"-w", "5", "-t", "nat", "-A", "MIHOMO_DNS_HANDLE", "-p", "tcp", "--dport", "53", "-j", "REDIRECT", "--to-ports", "1053"}},
		{"iptables", []string{"-w", "5", "-t", "nat", "-A", "PREROUTING", "-j", "MIHOMO_DNS"}},
	} {
	...
	for _, call := range calls {
		if call.name == "iptables" &&
			hasArgSequence(call.args, "-w", "5", "-t", "mangle") &&
			(hasArgSequence(call.args, "-A", "OUTPUT") || hasArgSequence(call.args, "-I", "OUTPUT")) &&
