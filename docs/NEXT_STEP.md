...
- 下一闭环进入阶段 5：代码结构收口。
- 当前已完成第八十五刀：manager sync unit 的通用 sections 已先接入 service。
- 目标不是扩功能，而是在不改变行为的前提下，继续消除 `render_manager_sync_service_unit` / `render_manager_sync_timer_unit` 周边的结构重复逻辑。

## 下一最小闭环

- 让 `render_manager_sync_timer_unit` 复用通用 sections helper
- 保持现有输出文本、顺序与退化行为不变
...
- manager sync unit 的通用 sections 拼装不再在 service/timer render 中重复
