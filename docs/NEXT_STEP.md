...
- 当前轮次又补稳了 `Setup` root guard、`Status` ensureAll 失败传播、`ImportLinks` / `RouterWizard` / `UpdateSubscriptions` 写回失败，以及 `Menu -> nodes/network/rules/service/audit` 主分发；该轮 `internal/app` 覆盖率快照曾从 96.3% 提升到 97.1%。
...
- 若继续施工，优先选择：
  - WebUI 首版已落地后，优先在真实 NAS 上用 `minimalist webui` 做一次只读 smoke；当前默认开放 LAN，先确认 `controller.secret` 足够强，再从 LAN 内设备访问 `http://<nas-lan-ip>:18080/`。
