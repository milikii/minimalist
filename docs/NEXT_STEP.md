...
- 下一闭环进入阶段 5：代码结构收口。
- 当前已完成第五十三刀：`prepare_project_install_tree` 的元数据清理已抽到共用 helper。
- 目标不是扩功能，而是在不改变行为的前提下，继续消除 `install_project_sync` / `disable_project_sync` 周边的结构重复逻辑。

## 下一最小闭环

- 提取 `cleanup_project_install_tree_metadata` 的 VCS 元数据清理 helper
- 保持现有输出文本、顺序与退化行为不变
...
- `cleanup_project_install_tree_metadata` 的 VCS 元数据清理不再与缓存垃圾清理内联耦合
