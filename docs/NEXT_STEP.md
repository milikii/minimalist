...
- 本轮已补上 `ApplyRules` 的 chain flush、已有 jump、DNS hijack、clear-rules 删除失败传播、仅显式代理分支清理失败传播、controller body read error、订阅删除缓存清理失败，以及 config/provider/runtime 的小边界测试；focused app 测试与全量 `go test ./...` 已通过。
