...
- 下一闭环进入阶段 5：代码结构收口。
- 当前已完成第七十五刀：`render_manager_sync_service_unit` 的 ConditionPathExists 内容已抽到共用 helper。
- 目标不是扩功能，而是在不改变行为的前提下，继续消除 `render_manager_sync_service_unit` / `render_manager_sync_timer_unit` 周边的结构重复逻辑。

## 下一最小闭环

- 提取 `render_manager_sync_service_unit` 的 Service 段内容 helper
- 保持现有输出文本、顺序与退化行为不变
...
- `render_manager_sync_service_unit` 的 Service 段内容不再内联在函数中
