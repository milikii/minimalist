...
- 下一闭环进入阶段 5：代码结构收口。
- 当前已完成第四十四刀：`write_manager_sync_units` 的 service unit 写入已抽到共用 helper。
- 目标不是扩功能，而是在不改变行为的前提下，继续消除 `install_project_sync` / `disable_project_sync` 周边的结构重复逻辑。

## 下一最小闭环

- 提取 `write_manager_sync_units` 的 timer unit 写入 helper
- 保持现有输出文本、顺序与退化行为不变
...
- `write_manager_sync_units` 的 timer unit 写入不再与封装函数内联耦合
