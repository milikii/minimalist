...
- 本轮已补上 `ApplyRules` 的 chain flush、已有 jump、DNS hijack、clear-rules 删除失败传播、仅显式代理分支清理失败传播、controller body read error、订阅删除缓存清理失败、订阅节点 provider-managed 保护，以及 config/provider/runtime 的小边界测试；focused app 测试与全量 `go test ./...` 已通过。
...
- 质量硬化已经把 `apply-rules` 的关键失败传播、route 编排幂等边界、rules-repo / provider 边界、config/state 缺省边界补到可测；当前没有新的功能缺口，后续只继续做低风险测试收口、push 和人工 cutover 等待。
