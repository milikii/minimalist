...
- 当前主线已进入阶段 5，已完成第八十三刀：manager sync unit 的通用 render 包装已抽到共用 helper。
...
- `write_manager_sync_service_unit` / `write_manager_sync_timer_unit` 的通用 render+write 编排仍可继续收口，阶段 5 后续可继续推进
