...
- 本轮验证结果：focused `go test ./internal/app -run 'TestControllerRuntimeSummary(FallsBackToLoopbackAndUsesSecret|ReturnsBodyReadError)|TestHealthcheckReportsController(ErrorWhenUnavailable|Summary)'` 与全量 `go test ./...` 通过。
