...
6. `status` 与 `runtime-audit` 的“当前模式”已优先读取 Mihomo REST API；`status` 还会读取 `/proxies` 输出最小策略组摘要，其余状态仍主要读取本地配置、systemd、端口和日志
...
- 阶段 4 只新增“运行态读取优先”能力
- 下一刀收口到 `runtime-audit` 的最小策略组运行态摘要读取与控制面回退
