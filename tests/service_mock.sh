...
#!/usr/bin/env bash
printf '%s\n' "$*" >> "${SYSTEMCTL_LOG:?}"
if [[ "$1" == "restart" && "$2" == "mihomo" ]]; then
  [[ -f "${CONFIG_FILE:?}" ]] || exit 61
  [[ -f "${SYSTEMD_UNIT:?}" ]] || exit 62
  [[ -f "${ROUTER_SYSCTL:?}" ]] || exit 63
  [[ -f "${MIHOMO_DIR:?}/Country.mmdb" ]] || exit 64
  [[ -f "${MIHOMO_DIR:?}/GeoSite.dat" ]] || exit 65
fi
exit 0
EOSYS
  chmod +x "${TMPDIR_CASE}/bin/systemctl"

  run_manager restart >/dev/null
...
test_runtime_audit_outputs() {
  ...
  grep -q '运行子状态: running' <<<"$output"
  grep -q '主进程 PID: 12345' <<<"$output"
  grep -q '累计 CPU 时间(ns): 123456789' <<<"$output"
  ...
  grep -q '过去 24 小时 error 数: 0' <<<"$output"
  grep -q '下次 Alpha 自动更新: Tue 2026-04-21 00:00:00 CST' <<<"$output"
  grep -q '下次定时重启: disabled' <<<"$output"
